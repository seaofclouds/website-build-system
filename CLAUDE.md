...
- **`46rem` is a literal in every media query that needs it** — currently thirteen of them; recount with `grep -h '@media.*46rem' content/static/*.css | wc -l` rather than trusting this number. It has to be a literal, because a media query can't read a custom property. It *can* assign one, which is how `--content-gutter` collapses without duplicating the geometry.
- **Both gutter layouts collapse at `72em`, and they must collapse together.** The grid is 72rem of fixed rem units, so below that neither can hold what it reserved without starving the body — `layout-margin` spends four columns on the right, `layout-nav` three on each side, and either way the body absorbs every lost pixel. `72em` is also where the change is invisible on the essay: inside the gutter the body is exactly `--measure`, so closing it moves nothing but the asides. The width appears twice — once in `layout.css` where the gutter closes (`width < 72em`), once in `content.css` where marginalia stops being lifted into it (`min-width: 72em`). They are complementary on purpose; make them overlap by one pixel and asides get absolutely positioned into a band that isn't there, on top of the body text. That has now happened twice, once per layout. Below 800px the nav additionally stacks and `--measure` becomes `100%` — all twelve columns are body.
...
`LICENSE` is MIT, `Copyright (c) 2019-2025 the Automerge contributors`. This repo is a derivative, so MIT **requires** that notice be retained — preserve the Automerge copyright line and add new ones alongside it rather than replacing it. Credit **Ivy Reese** as the original author, and never by any other name. The full name is what formal attribution uses — `LICENSE` and the README credits. Short-form "Ivy" is fine in running prose and in the site footer, and the README already does it. Link to `https://ivy.boo` or `https://github.com/ivyreese`; those are the only handles to use, in prose or inside a URL. The live-reload server in `system/server.ts` similarly credits its origin (`github.com/ivyreese/please-reload`); follow that precedent.
